...
class DPAreaRadar(DynamicProgrammingMixin, Radar):
    ...
    def get_next_frame_coords(self) -> [[int, int], [int, int]]:
        ...
        if current_x + invader_width - 1 < map_width:
            if current_y + invader_height - 1 < map_height:
                ...
                return [[current_y, current_x], [current_y + invader_height - 1, current_x + invader_width - 1]]
            else:
    ...
    def compute_frame_signal_bits_amount(self, frame_coords: [[int, int], [int, int]]) -> int:
        ...
        b_signal_bits = 0
        c_signal_bits = 0
        d_signal_bits = self.dp_matrix[bottom_x][bottom_y]
        if top_x > 0:
            c_signal_bits = self.dp_matrix[top_x - 1][bottom_y]
        if top_y > 0:
            b_signal_bits = self.dp_matrix[bottom_x][top_y - 1]
        if top_x > 0 and top_y > 0:
            a_signal_bits = self.dp_matrix[top_x - 1][top_y - 1]
